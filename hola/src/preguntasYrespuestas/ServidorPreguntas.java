...
public class ServidorPreguntas {
	...
	private static void enviarPreguntas(Socket socket, ArrayList<String> listado) {
		...
		for (String string : listado) {
			...
			int respueataCorrecta =Integer.parseInt(separador[0]);
			...
			if (respueataCorrecta == respuestaJugador) {
				contadorBien+=1;
			}else{
				contadorMal+=1;
			}
		...
		String mensajeRespuestas = "Ha respondido correctamente a: "+ contadorBien +"\nHa fallado: "+contadorMal + "\n¿Deseas volver a jugar?\n\n";
		construccionEnvioMensjes(mensajeRespuestas, socket);
