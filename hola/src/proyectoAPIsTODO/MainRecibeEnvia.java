...
public class MainRecibeEnvia {
	...
	private static final String INSTRUCCIONES = "c = Chiste, d = Dato, m = Mix \n"+ "cantidad = numero de peticiones\n"+"mail\n"+"<c o d>;<cantidad>;<mail>\n";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				/*buffer = INSTRUCCIONES.getBytes();
				socket.send(new DatagramPacket(buffer, buffer.length, ipCliente, puertoCliente));*/
				
				String [] separador = datosRecibidos.split(CARACTER_SEPARADOR);
				
				while(separador.length!=3) {
					buffer = MENSAJE_ERROR.getBytes();
					socket.send(new DatagramPacket(buffer, buffer.length, ipCliente, puertoCliente));
